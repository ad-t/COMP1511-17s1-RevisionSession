...
unsigned int stackSize(struct s_rep * stack) {
    // iterate through the items in the stack and count how many 
    // iterations occur
    if (stack == NULL) {
        return 0;
    }
...
// Verify that this function actually frees *everything* using valgrind!
// command: `valgrind ./<binary>` where <binary> is the compiled code.
